...
void customer_cart() {
    ...
    else //链表不为空
    {
        ...
        printf("\t*****************************************************************************************\n");
        printf("\t\t总价格:%.2f\n", sum);
        printf("\t\t输入1结账，输入2删除指定商品，输入0返回购物系统主菜单...\n");
        if (whethermember) {
            printf("\t\t会员打折后总价格:%.2f\n", sum * 0.95);
        }
        else {
...
void customer_cart_delete() {
    ...
    printf("\t 请输入你要删除的商品编号:");
    scanf("%d", &delete_id);
    Cart* p_find; //定义一个指针变量
    p_find = cart_head; //将头指针赋值给p
    while (p_find != NULL) {
        if (p_find->id == delete_id) {
            if (p_find == cart_head) {
                cart_head = p_find->next;
                printf("\t 删除成功，按任意键返回");
                getchar();
                getchar();
                ...
                Cart* find_head, * find_tail;
                find_head = cart_head;
                find_tail = find_head->next;
                while (find_tail != NULL) {
                    if (find_tail->id = delete_id) {
                        find_head->next = find_tail->next;

                        Goods* stock_add;
                        stock_add = head;
                        while (stock_add != NULL)
                        {
                            if (stock_add->id = delete_id) {
                                stock_add->stock += find_tail->number;
                                break;
                            }
                            else {
                                stock_add = stock_add->next;
                            }
                        }
                        free(find_tail);
                        printf("\t 删除成功,按任意键返回");
...
void registermembers()
{
    ...
    do {
        if (!strcmp(a.key, temp))
        {
            ...
            printf("会员注册成功，已成功绑定手机，请按任意键返回登录界面进行登录。。。\n");
            getchar();
            getchar();
            system("cls");
            FILE* ph;
...
void memberfind()  //按结构体的大小，一次写入或者读入1个结构体大小
{
    ...
    printf("\t\t\t\t\t欢迎来到找回密码界面，密码找回需要对您进行身份验证\n");
    Sleep(1000);
